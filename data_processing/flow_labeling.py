...
This script assigns ground-truth labels to flows generated by CICFlowMeter, which
does not produce labels by default. It cross-references each flow's timestamp, source and destination IPs
with a predefined dictionary of attack windows, attacker IPs, and victim IPs derived from the CICIDS2018
documentation. The result is a labeled CSV file with each flow marked as Benign or with its corresponding
attack type.
"""
